...
export default router.post(
  ...
  async (req, res) => {
    const { key } = req.body;
    const vendorConfigData = await u.db("o_vendorConfig").where("id", "toonflow").andWhere("userId", (req as any).user.id).first();
    ...
    try {
      ...
      if (resText.text) {
        await u.db("o_agentDeploy").where("key", "scriptAgent").andWhere("userId", (req as any).user.id).update({
          model: "claude-sonnet-4-6",
          modelName: "toonflow:claude-sonnet-4-6",
          vendorId: "toonflow",
        });
        await u.db("o_agentDeploy").where("key", "productionAgent").andWhere("userId", (req as any).user.id).update({
          model: "claude-sonnet-4-6",
          modelName: "toonflow:claude-sonnet-4-6",
          vendorId: "toonflow",
        });
        await u.db("o_agentDeploy").where("key", "universalAi").andWhere("userId", (req as any).user.id).update({
